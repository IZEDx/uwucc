 ...
 * ```
 */
const pipe = <T extends [Fn, ...Fn[]]>(...funs: PipeReturn<T> extends never ? never : T) =>
	((...args) => {
		let nextArgs: unknown[] = args;
		const fns: ((...a: any) => any)[] = funs;
		for (let i = 0; i < fns.length; i++) {
			const [result] = (nextArgs = [fns[i](...nextArgs)]);
			if (result && result instanceof Promise) return resolveAsync(result, funs.slice(i + 1));
		}
...
type CheckPipe<F extends Fn[], D extends FunDef[], Async extends boolean = false> = F extends [
	any,
	any,
	...any[],
]
